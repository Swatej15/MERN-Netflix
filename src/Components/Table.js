        ...
        <tr key={i}>
            ...
            <td className={`${Text} float-right flex-rows gap-2`}>
                {admin ? (
                        ...
                        <Link to={`/movies/${movie?.name}`} className='bg-subMain text-white rounded flex-colo w-6 h-6'>
                        <GoEye />
                        </Link> 
                        </>
                    )}
  ...
  return (
    <div className='overflow-x-scroll overflow-hidden relative w-full'>
        <table className='w-full table-auto border border-border divide-y divide-border'>
            ...
            <tbody className='bg-min divide-y divide-gray-800'>
                {data.map((item,i ) => Rows(item,i,admin))}
            </tbody>
        </table> 
    </div>
  )
